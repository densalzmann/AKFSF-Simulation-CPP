...
void KalmanFilter::handleLidarMeasurement(LidarMeasurement meas, const BeaconMap& map)
{
    if (isInitialised())
    {
        ...
        {           
            ...
            VectorXd r = Vector2d::Zero();
            r << map_beacon.x - state(0), map_beacon.y - state(1);
            double r_length = sqrt(r(0) * r(0) + r(1) * r(1));
            Vector2d meas_pred = Vector2d::Zero();
            meas_pred << r_length, wrapAngle(atan2(r(1),r(0)) - state(2));

            MatrixXd H = MatrixXd(2,4);
            H << -r(0)/r_length, -r(1)/r_length, 0, 0, r(1)/r_length/r_length, -r(0)/r_length/r_length, -1, 0;
