...
void KalmanFilter::handleLidarMeasurement(LidarMeasurement meas, const BeaconMap& map)
{
    if (isInitialised())
    {
        ...
        {           
            ...
            // Vector2d measure = Vector2d::Zero();
            // measure << meas.range, meas.theta;
            
            // // VectorXd z_hat = Vector2d::Zero();
            // // z_hat << map_beacon.x - state[0], map_beacon.y - state[1];
            // double delta_x = map_beacon.x - state[0];
            // double delta_y = map_beacon.y - state[1];
            // double r_length = sqrt(delta_x * delta_x + delta_y * delta_y);
            // double zhat_theta = wrapAngle(atan2(delta_y,delta_x) - state[2]);
            // Vector2d meas_pred = Vector2d::Zero();
            // meas_pred << r_length, zhat_theta;
            ...
            double r_length = sqrt(delta_x*delta_x + delta_y*delta_y);  
            double zhat_theta = wrapAngle(atan2(delta_y,delta_x) - state[2]);
            z_hat << r_length, zhat_theta;

            MatrixXd H = MatrixXd(2,4);
            // H << -delta_x/r_length, -delta_y/r_length, 0, 0, delta_y/r_length/r_length, -delta_x/r_length/r_length, -1, 0;
             H << -delta_x/r_length,-delta_y/r_length,0,0,delta_y/r_length/r_length,-delta_x/r_length/r_length,-1,0;
...
void KalmanFilter::predictionStep(GyroMeasurement gyro, double dt)
{
    if (isInitialised())
    {
        ...
        // ----------------------------------------------------------------------- //
        // ENTER YOUR CODE HERE
        state(0) = state(0) + dt * state(3) * cos(state(2));
        state(1) = state(1) + dt * state(3) * sin(state(2));
...
void KalmanFilter::handleGPSMeasurement(GPSMeasurement meas)
{
    ...
    else
    {
        ...
        state(0) = meas.x;
        // state(0) = 0; //test for bad initial conditions
        state(1) = meas.y;
        // state(1) = 0; //test for bad initial conditions
